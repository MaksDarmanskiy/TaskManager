...
export default function DashboardPage() {
  ...
  return (
    <Box>
      <Typography variant="h4" fontWeight={700} mb={1}>Дашборд</Typography>
          ...
          <StatCard accent={item.accent} key={item.label}>
            <CardContent>
                <Box display="flex" justifyContent="space-between" alignItems="flex-start">
                  <Box>
                    <Typography variant="h3" fontWeight={700} color={item.color}>{item.value}</Typography>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '5fr 7fr' }, gap: 3 }}>
        ...
        <Box>
          <Card sx={{ height: 300 }}>
            <CardContent>
              <Typography variant="h6" fontWeight={600} mb={2}>Задачі по статусах</Typography>
        ...
        <Box>
          <Card sx={{ height: 300 }}>
            <CardContent>
              <Typography variant="h6" fontWeight={600} mb={2}>Задачі по пріоритетах</Typography>
              <ResponsiveContainer width="100%" height={220}>
        ...
        <Box sx={{ gridColumn: '1 / -1' }}>
          <Card>
            <CardContent>
              <Typography variant="h6" fontWeight={600} mb={3}>Прогрес по проєктах</Typography>
              {stats.projectStats.length === 0 ? (
                <Typography color="text.secondary">Ще немає проєктів</Typography>
              ) : (
                      ...
                      <Box key={p.id} sx={{
                        ...
                        <Box display="flex" justifyContent="space-between" alignItems="center" mb={1.5}>
                          <Box display="flex" alignItems="center" gap={1}>
                            ...
                            <Typography variant="body2" fontWeight={600} noWrap title={p.title} sx={{ maxWidth: 150 }}>
                              {p.title}
                            </Typography>
                          </Box>
                          <Typography variant="caption" fontWeight={700} sx={{ color: p.color || '#171717' }}>
                            {progress}%
                          </Typography>
                        </Box>
